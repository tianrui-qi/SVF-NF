...
if __name__ == "__main__":
    ...
    plot_deconvolution(num_z, g, out_dir, tag='Experimental PSF Deconvolution')
    g_exper = g
    ...
    # show the deconvolved results
    plot_deconvolution(num_z, g, out_dir, tag='Retrieved Phase Deconvolution')
    g_retri = g
        ...
        if epoch < args.init_epochs:
            ...
            if args.show_inter_imgs and (epoch + 1) % args.display_freq == 0:
                ...
                for i in range(num_z):
                    ...
                    plt.imshow(np.clip(g_est[0, i].detach().cpu().numpy(), 0, 1e3), cmap='gray')
                    plt.axis('image')
                    plt.axis('off')
                    plt.title(f"z{i}")
